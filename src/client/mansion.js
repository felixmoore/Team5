...
class Mansion extends Phaser.Scene {
  ...
  create() {
    ...
    const living_room_tileset = map.addTilesetImage(
      "living_room",
      "living_room"
    );
    ...
    const infoBg = this.add
      .rectangle(0, 0, map.widthInPixels, 40, 0x008000)
      .setScrollFactor(0);
  ...
  update() {
    if (startGame) {
      const testRect = this.add
        .rectangle(1988, 816, 500, 300, 0x008000)
        .setScrollFactor(0);
...
function updateSpriteColour(self, data, colour) {
  let roleText = self.add
    .text(0, 0, "Player role: ")
    .setScrollFactor(0)
    .setFontFamily("Arial");
...
function createTimer(self) {
  ...
  self.timerText = self.add
    .text(630, 670, "Countdown: " + formatTime(self.initialTime))
    .setScrollFactor(0)
    .setFontFamily("Arial");
