...
class Mansion extends Phaser.Scene {
  ...
  preload() {
    ...
    this.load.image("walls_floors",
                    "public/assets/tilesets/Tilesets_48x48.png");
  ...
  create() {
    ...
    // of colliding tiles faceColor: new Phaser.Display.Color(40, 39, 37, 255)
    ...
    this.cameras.main.setBounds(0, 0, map.displayWidth, map.displayHeight)
        .setZoom(1); // TODO looks better at 1.5, but breaks the info/timer bar?
    ...
    this.socket.on(
        'currentPlayers',
        (players) => { // Loads all currently connected players.
          allPlayers = players;
          Object.keys(players).forEach((index) => {
            if (players[index].id === this.socket.id) {
              ...
              this.physics.add.collider(this.player, worldLayer, null, null,
                                        this);
    ...
    const infoBg = this.add.rectangle(0, 0, map.widthInPixels, 40, 0x008000)
                       .setScrollFactor(0);
    ...
    bgm.setLoop(true);
    // bgm.play(); //commented out for now to stop it being annoying while
    // debugging
  ...
  update() {
    if (startGame) {
      const testRect =
          this.add.rectangle(1988, 816, 500, 300, 0x008000).setScrollFactor(0);
    ...
    if (self.player.x > currentX && self.player.x < currentXX &&
      ...
      if (current.linkedTo != null) { // teleport if object is a portal
        ...
        if (current.clues != null) { // clue collision
          self.collectClue();
          clue_collect.play();
          // self.game.state.load('wordsearch',
...
function drawObjects(self, objects) {
  const clueText = self.add.text(650, 0, 'Clues collected:  ')
                       .setScrollFactor(0)
                       .setFontFamily('Arial');
...
function updateSpriteColour(self, data, colour) {
  let roleText = self.add.text(0, 0, 'Player role: ')
                     .setScrollFactor(0)
                     .setFontFamily('Arial');
...
function createTimer(self) {
  ...
  self.timerText =
      self.add.text(630, 670, 'Countdown: ' + formatTime(self.initialTime))
          .setScrollFactor(0)
          .setFontFamily('Arial');
