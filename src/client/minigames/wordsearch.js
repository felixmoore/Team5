...
const game = new Phaser.Game(config); // instantises game with config properties
...
function update () {
  ...
  if (me.guessing) {
    ...
    const hoverX = me.game.input.x;
    const hoverY = me.game.input.y;

    // checks pointer location in relation to the grid
    const hoverPosX = Math.floor((hoverX - me.leftBuffer) / me.tileWidth);
    const hoverPosY = Math.floor((hoverY - me.topBuffer) / me.tileHeight);
    ...
    if (hoverPosX >= 0 && hoverPosX < me.tileGrid[0].length && hoverPosY >= 0 && hoverPosY < me.tileGrid[0].length) {
      const hoverTile = me.tileGrid[hoverPosX][hoverPosY]; // grabs the tile being hovered

      // defines the pointer hover bounds of the tile
      const tileLeftPosition = me.leftBuffer + (hoverPosX * me.tileWidth);
      const tileRightPosition = me.leftBuffer + (hoverPosX * me.tileWidth) + me.tileWidth;
      const tileTopPosition = me.topBuffer + (hoverPosY * me.tileHeight);
      const tileBottomPosition = me.topBuffer + (hoverPosY * me.tileHeight) + me.tileHeight;
    ...
    if (me.currentWord.length > 0) {
      ...
      for (let i = 0; i < me.currentWord.length; i++) {
...
function initTiles () { // initialise tile function for initialising the tileGrid
  ...
  for (let i = 0; i < me.tileGrid.length; i++) { // Loops each column of the grid
    for (let j = 0; j < me.tileGrid.length; j++) { // loops each position in a column
      const tile = me.addTile(i, j); // adds the tile to the game at given position
...
function addTile (x, y) {
  ...
  const tileLetter = me.tileLetters[me.random.integerInRange(0, me.tileLetters.length - 1)];
  const tileColor = me.tileColors[me.random.integerInRange(0, me.tileColors.length - 1)];

  const tileToAdd = me.createTile(tileLetter, tileColor); // stores the properties of a tile to be added

  // adds tile to given X position
  const tile = me.tiles.create(me.leftBuffer + (x * me.tileWidth) + me.tileWidth / 2, 0, tileToAdd);
...
function createTile (letter, color) {
  ...
  const tile = me.game.add.bitmapData(me.tileWidth, me.tileHeight);
...
function createScore () {
  const me = this;
  const scoreFont = '100px Arial'; // creates font variable for displaying score
...
function updateTimer () {
  ...
  const cropRect = new Phaser.Rectangle(0, 0, (me.remainingTime / me.fullTime) * me.game.width, me.timeBar.height);
