...
class Drag extends Phaser.Scene {
    ...
    create() {
        ...
        let playerCursors = this.physics.add.group();
        Object.keys(allPlayers).forEach((index) => {
            if (allPlayers[index].id != socket.id) {
                let otherPlayer = this.add.sprite(400, 300, 'cursor').setTint();
        ...
        this.input.on('drag', function (pointer, gameObject, dragX, dragY) {
            let originalCoordinates = [gameObject.x, gameObject.y];
            gameObject.x = dragX;
            gameObject.y = dragY;
...
function tryLock(key, lock, socket) {
    ...
    if (key.tintTopLeft === lock.tintTopLeft) {
        ...
        //TODO emit event disabling key/lock on server
    }
}
